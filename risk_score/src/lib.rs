...
impl RiskTierContract {
    ...
    /// Return the current admin address, or panic if not initialized.
    ...
    pub fn set_risk_tier(env: Env, user: Address, score: u32, tier: Symbol, chosen_tier: Symbol) {
        // --- Access control: admin OR the user themselves must authorize ---
        //
        // Soroban v22 uses a declarative auth model: the caller provides signed
        // authorization proofs. We require the user's auth here; the admin can
        // authorize on behalf of any user by being set as an authorized invoker
        // in the Soroban auth tree (the standard composable pattern).
        //
        // We also verify the contract has been initialized so that an uninitialized
        // contract cannot be called at all.
        let admin_key = Symbol::new(&env, ADMIN_KEY);
        assert!(
            env.storage()
                .instance()
        ...
        // Require authorization from the user being updated.
        // The admin can satisfy this requirement by providing auth for the user
        // address via Soroban's auth framework.
        user.require_auth();
...
mod tests {
    ...
    /// Helper: register contract and initialize with a fresh admin.
    /// mock_all_auths() bypasses auth checks so tests focus on logic.
    ...
    // ── existing functional tests (updated to use setup()) ───────────────────
