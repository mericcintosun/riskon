...
import BlendDashboard from "../components/BlendDashboard";
import EnhancedLiquidityPools from "../components/EnhancedLiquidityPools";
...
export default function RiskScoringApp() {
  ...
  const performWalletAnalysis = async () => {
    ...
    try {
      ...
      const analysisResult: any = await performAutoRiskAnalysis(walletAddress);
  ...
  const submitRiskScore = async () => {
    ...
    try {
      ...
      const result: any = await writeScoreToBlockchainEnhanced({
        ...
        score: riskScore,
        chosenTier: riskScore <= 30 ? "TIER_1" : riskScore <= 70 ? "TIER_2" : "TIER_3",
      });
