...
import { saveRiskData } from "../../lib/storage/db";
...
export async function writeScoreToBlockchain({
  ...
  try {
    ...
    const transaction = new TransactionBuilder(sourceAccount, {
      fee: (Number(BASE_FEE) * 100000).toString(), // High fee for Soroban contracts
    ...
    if (
      ...
      if (result.errorResult) {
        const errorCode =
          (result.errorResult as any)._attributes?.result?._switch?.name ||
          (result.errorResult as any)._attributes?.result?._switch?.value;
      ...
      if (result.hash) {
        ...
        const possibleHash =
          (result as any).id || (result as any).txHash || (result as any).transactionHash;
        if (possibleHash) {
          return {
            successful: true,
            hash: possibleHash,
...
export async function readRiskTierFromBlockchain(address) {
  try {
    ...
    if ((simulationResponse as any).error) {
      return null;
    }

    // Extract result from simulation
    const result = (simulationResponse as any).result?.retval;
 ...
 * @returns {Promise<Object>} Transaction result
 */
export async function storeScoreInAccountData({ kit, address, score, chosenTier }: { kit: any; address: any; score: any; chosenTier?: any }) {
  try {
...
export async function writeScoreToBlockchainEnhanced({
  ...
  try {
    ...
    if (result.successful) {
      ...
      throw new Error((result as any).error || "Blockchain transaction failed");
    }
  } catch (error) {
