...
export function WalletProvider({ children }) {
  ...
  const connectWallet = async (walletId = null) => {
    if (!kit) {
      const error = new Error("Wallet kit not ready yet. Please try again in a moment.");
      throw error;
    }
    ...
    try {
      if (walletId) {
        ...
        return new Promise((resolve, reject) => {
          try {
              ...
              onWalletSelected: async (option) => {
                try {
                  ...
                  if (error.message?.includes("User rejected")) {
                    ...
                    reject(new Error("Network error during wallet connection. Please check your internet connection."));
                  } else if (error.message?.includes("not found") || error.message?.includes("undefined")) {
                    reject(new Error("Wallet extension not found. Please install the wallet extension and refresh the page."));
                  } else {
                    reject(new Error(`Wallet connection failed: ${error.message || "Unknown error"}`));
                  }
                }
              },
              onClosed: (error) => {
            ...
            reject(new Error(`Failed to open wallet selection: ${error.message}`));
          }
        });
      }
    } catch (error) {
      ...
      if (error.message?.includes("timeout")) {
        enhancedError = new Error("Wallet connection timed out. Please try again.");
      } else if (error.message?.includes("network") || error.message?.includes("fetch")) {
        enhancedError = new Error("Network error occurred. Please check your internet connection and try again.");
      } else if (error.message?.includes("permission") || error.message?.includes("denied")) {
        enhancedError = new Error("Wallet permission denied. Please allow access and try again.");
      } else if (error.message?.includes("not installed") || error.message?.includes("not found")) {
        enhancedError = new Error("Wallet extension not installed. Please install a Stellar wallet extension.");
      } else {
        enhancedError = new Error(`Wallet connection error: ${error.message || "Unknown error occurred"}`);
      }
