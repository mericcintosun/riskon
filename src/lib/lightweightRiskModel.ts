...
function calculateConfidence(features) {
  ...
  const featureValues: number[] = Object.values(features) as number[];
  const mean =
...
function generateExplanation(riskScore, featureImportance) {
  ...
  const sortedFeatures = Object.entries(featureImportance).sort(
    (a, b) => Math.abs((b[1] as any).impact) - Math.abs((a[1] as any).impact)
  );

  sortedFeatures.slice(0, 2).forEach(([feature, data_]) => {
    const data = data_ as any;
...
function generateRecommendations(featureImportance) {
  ...
  Object.entries(featureImportance).forEach(([feature, data_]) => {
    const data = data_ as any;
