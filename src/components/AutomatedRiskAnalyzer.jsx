  ...
  const runAutomatedAnalysis = async () => {
    ...
    try {
      ...
      if (!dataQuality.isGood) {
        setTimeout(() => {
          toast.warning(
            `⚠️ Limited history: ${dataQuality.txCount} txs, ${dataQuality.counterpartyCount} counterparties — score may be less accurate`,
            { duration: 6000 }
          );
        }, 1000);
      }
          ...
          <div className="bg-orange-50 dark:bg-orange-900/20 rounded-xl border border-orange-200 dark:border-orange-800 p-4">
            <div className="flex items-start space-x-3">
              ...
              <div className="text-sm text-orange-800 dark:text-orange-200">
                <strong>Limited Transaction History:</strong> Score accuracy is lower because this wallet has{" "}
                <strong>{analysisData.dataQuality.txCount ?? 0} transaction{analysisData.dataQuality.txCount !== 1 ? "s" : ""}</strong>{" "}
                and{" "}
                <strong>{analysisData.dataQuality.counterpartyCount ?? 0} unique counterpart{analysisData.dataQuality.counterpartyCount !== 1 ? "ies" : "y"}</strong>.
                {" "}Send or receive a few more payments to improve analysis quality.
              </div>
            </div>
          </div>
        )}
